...
class LidarSelector {
  public:
    ...
    void detect(cv::Mat img, PointCloudXYZI::Ptr pg, const std::vector<V3D>& V, const std::vector<V3D>& V_world);
    float CheckGoodPoints(cv::Mat img, V2D uv);
    void addFromSparseMap(cv::Mat img, PointCloudXYZI::Ptr pg);
    void addSparseMap(cv::Mat img, PointCloudXYZI::Ptr pg, const std::vector<V3D>& V);
    ...
    float UpdateState(cv::Mat img, float total_residual, int level, const std::vector<V3D>& V);
    double NCC(float* ref_patch, float* cur_patch, int patch_size);
    void loadParameters(ros::NodeHandle& n);
    void projectionJacobian(const V3D& p, MD(2,3)& du_dp);

    void ComputeJ(cv::Mat img, const std::vector<V3D>& V);
    void reset_grid();
